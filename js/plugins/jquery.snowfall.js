...
(function($) {
	$.snowfall = {
        ...
        start: function(options, $wrapper) {
            ...
            var $snowfall = $('<div class="flake" />').css({'position': 'absolute', 'top': '-50px'});
            ...
            $.snowfall.intervals.push(setInterval(function() {
                ...
                $snowfall.clone().html(((Math.random() <= 0.5) ? 1 : 0) ? '<i class="far fa-circle"></i>' : '<i class="fas fa-times"></i>').appendTo($wrapper).css({
                    'left': startPosition,
